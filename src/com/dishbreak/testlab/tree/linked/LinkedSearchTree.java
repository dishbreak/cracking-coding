...
public class LinkedSearchTree {
    ...
    public LinkedTreeNode getCommonAncestor(LinkedTreeNode o1, LinkedTreeNode o2) {
        LinkedTreeNode result = null;
        result = search(o1, (LinkedTreeNode node) -> {
            return node == o2;
        });
        ...
        TreeTest findOther = (LinkedTreeNode node) -> {
            return node == o1;
        };
        ...
        if (o2.parent() != null) {
            ...
            TreeGetter getter;
            if (o2.isLeftChild()) {
                test = (LinkedTreeNode node) -> {
                    return node.right() != null;
                };
                getter = (LinkedTreeNode node) -> {
    ...
    public boolean isSubtree(LinkedSearchTree tree) {
        ...
        LinkedTreeNode result = search((LinkedTreeNode item) -> {
    ...
    private boolean bfsMatch(LinkedList<LinkedTreeNode> resultQueue, LinkedList<LinkedTreeNode> nodeList) {
        ...
        if (!allNodesVisited)
            return false; // this means the input queues weren't the same size.
        if (!bothQueuesSameSize)
            return false; // this means some of the input nodes were actually
                          // leaves.
        if (!bothQueuesEmpty)
            return bfsMatch(nextResultQueue, nextNodeQueue);
        else
            return true; // this means that the input was just leaves.
